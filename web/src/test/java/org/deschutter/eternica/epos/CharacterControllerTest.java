...
public class CharacterControllerTest {
    ...
    public void testCharacterView_LoggedIn_HasUserAccess_AccessGranted() throws Exception {
        when(characterService.getCharacter(1L)).thenReturn(new CharacterDTO(1L,"CharacterName1"));
        mockMvc.perform(get("/epos/character/1")).andExpect(status().isOk()).andExpect(view().name("epos/character")).andExpect(model().attribute("character",allOf(hasProperty("id",is(1L)),hasProperty("characterName",is("CharacterName1")))));
    }
}
